...
/**********
 * Score that decrements everytime you hit play
 */
 ...
 */
public class SayBack extends JPanel {
    ...
    class Recorder extends JPanel implements ActionListener, ControlContext {
        ...
        JButton playB, captB, pausB, loadB, reverseB;
...
/*	                File file = new File(System.getProperty("user.dir"));
	        ...
	        if (file != null && file.isFile()) {
	            try {
	                ...
	                if (updateComponents) {
	                    ...
	                    samplingGraph.createWaveForm(null,duration);
	                }
	            } catch (Exception ex) { 
	    ...
	    public void createAudioInputStream(InputStream file, boolean updateComponents) {
	        
            try {
                errStr = null;
               // AudioFormat format = audioInputStream.getFormat();
                audioInputStream = new AudioInputStream(file, formatControls.getFormat(), 2097152);
	    ...
	    public void reverse(){
	    	...
	    	int length = frameSizeInBytes * (int)playbackInputStream.getFrameLength();
	    	byte[] data = new byte[length];
	    	byte[] reverse = new byte[length];
            int numBytesRead = 0;
            
            try {
            	//playbackInputStream.reset();
				numBytesRead = playbackInputStream.read(data, 0, length);
	    	...
	    	for(int i = 0; i < playbackInputStream.getFrameLength(); i++)
	    		for(int j = 0; j < frameSizeInBytes; j++)
	    			if(length - (i + 1) * frameSizeInBytes + j > -1 && i * frameSizeInBytes + j < length)
	    				reverse[length - (i + 1) * frameSizeInBytes + j] = data[i * frameSizeInBytes + j];
	    			else
	    				return;
	    	
	    	
            ByteArrayInputStream bais = new ByteArrayInputStream(reverse);
            audioInputStream = new AudioInputStream(bais, format, reverse.length / frameSizeInBytes);
            ...
            samplingGraph.repaint();
           // samplingGraph.createWaveForm(reverse);
	    }
	            ...
	            while (thread != null) {
	                try {
	                    ...
	                    if(forward) {
	                        ...
	                        FrameBuffer frameStream = new FrameBuffer(playbackInputStream);
	                        int frame = frameStream.numberFrames() - 1;
	                        while (frame >= 0) {
	                             line.write(frameStream.getFrame(frame), 0, frameSizeInBytes);
	                             frame--;
	                        }
	                    }
	                } catch (Exception e) {
	    ...
	    class Capture implements Runnable {
            ...
            public Capture() {
	            ...
	            //First time the line starts, there's a 1s delay.
	            //This will start and stop the line on load up
	            initLine();
            }
	        ...
	        public void run() {
	            ...
	            while (thread != null) {
	                ...
	                tmpOut.write(data, 0, numBytesRead);
	            }
	            ...
	            // Reinitialize data for next capture
	            tmpOut = new ByteArrayOutputStream();
	            data = new byte[bufferLengthInBytes];
	            ...
	            samplingGraph.createWaveForm(audioBytes, duration);
	        }
	    ...
	    class SamplingGraph extends JPanel implements Runnable {
	        ...
	        public void createWaveForm(byte[] audioBytes, double seconds) {
	        	duration = seconds;
	        ...
	        public void paint(Graphics g) {
	            ...
	            if (errStr != null) {
	                if(forward) g2.setColor(jfcBlue); else g2.setColor(red);
    ...
    public static void main(String s[]) {
        SayBack sayback = new SayBack();
        sayback.open();
        JFrame f = new JFrame("SayBack");
        f.addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent e) { System.exit(0); }
        });
        f.getContentPane().add("Center", sdrawkcab);
        f.pack();
